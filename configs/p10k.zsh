# Generated by Powerlevel10k configuration wizard on 2025-08-12 at 10:24 PDT.
...
# slanted separators, round heads, round tails, 2 lines, solid, right frame,
...
() {
  ...
  [[ $USER == "ec2-user" ]] && typeset -g POWERLEVEL9K_OS_ICON_BACKGROUND=9
  # Custom icon.
  ...
  # Show 'ec2' instead of 'ec2-user@ip-...' on EC2 instances
  [[ $USER == "ec2-user" ]] && typeset -g POWERLEVEL9K_CONTEXT_{REMOTE,REMOTE_SUDO}_TEMPLATE='ec2'
  ...
  function prompt_shpool() {
    [[ -n "$SHPOOL_SESSION_NAME" ]] || return
    local name=$SHPOOL_SESSION_NAME
    if (( ${#name} > 20 )); then
      name="${name[1,8]}...${name[-9,-1]}"
    fi
    p10k segment -b 6 -f 0 -t "$name"
  }

  function instant_prompt_shpool() {
    prompt_shpool
  }
